...
3.  **Deploy the Bicep template:**
    Execute the following Azure CLI command. This will deploy resources at the Subscription scope and create the resource group `rg-tf-backend` (or a name of your choice).

    ```bash
    az deployment sub create --location swedencentral --template-file main.bicep --parameters resourceGroupName='rg-tf-backend'
    ```

4.  **Permissions:**
    The Bicep template automatically assigns the `Contributor` and `User Access Administrator` roles to the Managed Identity at the **Subscription** scope. No manual role assignment is needed.
